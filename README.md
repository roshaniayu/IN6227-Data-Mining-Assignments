# IN6227 Data Mining Assignments

Full name: Roshani Ayu Pranasti<br/>
Matric no: G2504973A<br/>
M.Sc. in Information Systems<br/>
Wee Kim Wee School of Communication and Information<br/>
Nanyang Technological University<br/>
2025

> Every assignment is worked on in each related folder.

## Assignment 1

**Objective**: Train and test classification models (I use Naive Bayes and k-NN) and compare both using the provided dataset.<br/>
**Dataset**: Census Income dataset from the UCI Machine Learning Repository (https://archive.ics.uci.edu/ml/datasets/Census+Income)

## Assignment 2

**Objective**: Use an association rules mining tool (I use Orange 3) and compare it to the brute-force approach.<br/>
**Dataset**: Bakery Sales Dataset retrieved from Kaggle (https://www.kaggle.com/datasets/akashdeepkuila/bakery)
